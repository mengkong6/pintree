const global = {
    iconService: 'https://favicon.im/',
    address: 'www.sapi.run'
};
...
function createCard(title, url, icon) {
    ...
    card.onclick = () => window.open(url + '?from=www.sapi.run', '_blank'); // Make the whole card clickable
...
function showNoResultsMessage() {
    ...
    title.innerText = '哎呀，这里没有什么可看的内容！';

    const message = document.createElement('p');
    message.className = 'text-gray-500 mt-2';
    message.innerText = '尝试其他搜索或在其他地方发现更多有趣的内容！';
